...
private:
    ...
    static void print_inode_info(Disk *disk, Inode *inode);
    static bool formatted(Disk *disk, Block superBlock);
    static void initialize_inode(Inode *inode);
    static ssize_t load_inode(Disk *disk, size_t inumber, Inode *node);
    static bool save_inode(Disk *disk, size_t inumber, Inode *node);
    size_t find_free_block();
    static ssize_t write_data(Disk *disk, size_t blockNumber, char *data, size_t length, size_t offset);
    static void init_data_block(Disk *disk, size_t blockNumber);

    // TODO: Internal member variables
    size_t numOfBlocks;
    size_t numOfInodeBlocks;
    size_t numOfInodes;
